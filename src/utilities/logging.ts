...
export class logging {
  ...
  static error(message: string) {
    if (this.level >= levels.error) { console.error('%s \x1b[31m%s\x1b[0m', this.time(), message) }
  }
  ...
  static warn(message: string) {
    if (this.level >= levels.warn) { console.warn('%s \x1b[33m%s\x1b[0m', this.time(), message) }
  }
  ...
  static info(message: string) {
    if (this.level >= levels.info) { console.info('%s \x1b[34m%s\x1b[0m', this.time(), message) }
  }
  ...
  static success(message: string) {
    console.log('%s \x1b[32m%s\x1b[0m', this.time(), message)
  }
